...
class DefaultController extends AbstractController
{
	...
	public function execute()
	{
		...
		$vClass = '\\BabDev\\View\\' . ucfirst($vName) . '\\' . ucfirst($vName) . ucfirst($vFormat) . 'View';
		$mClass = '\\BabDev\\Model\\' . ucfirst($vName) . 'Model';

		// If a model doesn't exist for our view, revert to the default model
		if (!class_exists($mClass))
		{
			$mClass = '\\BabDev\\Model\\DefaultModel';
		...
		{
			$vClass = '\\BabDev\\View\\Default' . ucfirst($vFormat) . 'View';
